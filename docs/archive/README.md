# Archive — Prior Project Direction

The four files in this directory document a project direction (codename "Whale-Sense" — an autonomous DeFi trading agent) that was scoped and stress-tested through `/brainstorm` → `/moat-check` → `/prfaq` on 2026-05-11.

That direction was retired on the same day after better signal arrived about the target reader's actual product space (RWA fintech, not DeFAI). The archive is retained as evidence of the decision process:
